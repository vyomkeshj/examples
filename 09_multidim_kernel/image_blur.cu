...
void check_blurred_image(unsigned char *image, int width, int height, int blur_size) {
    ...
    for (int r = 0; r < height; r++) {
        ...
        for (int c = 0; c < width; c++) {
            ...
            if (width % pattern_size <= blur_size && c >= (width / pattern_size) * pattern_size ||
                height % pattern_size <= blur_size && r >= (height / pattern_size) * pattern_size)
                true_val = 0;
            else
                true_val = 255 / (sx * sy);
